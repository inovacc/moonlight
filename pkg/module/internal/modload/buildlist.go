...
import (
	...
	mvs2 "github.com/inovacc/moonlight/pkg/module/internal/mvs"
	par2 "github.com/inovacc/moonlight/pkg/module/internal/par"
	"log"
	"os"
	"reflect"
	"runtime"
...
func (rs *Requirements) initVendor(vendorList []module.Version) {
	rs.graphOnce.Do(func() {
		...
		mg := &ModuleGraph{
			g: mvs2.NewGraph(cmpVersion, roots),
		}
		...
		if rs.pruning == pruned {
			...
			for _, m := range vendorList {
				if v, ok := rs.rootSelected(m.Path); !ok || v != m.Version {
					log.Fatalf("go: vendored module %v should be required explicitly in go.mod", m)
					inconsistent = true
				}
			}
			if inconsistent {
				log.Fatal(errGoModDirty)
			}
...
type ModuleGraph struct {
	g         *mvs2.Graph
	loadCache par2.ErrCache[module.Version, *modFileSummary]
...
func readModGraph(ctx context.Context, pruning modPruning, roots []module.Version, unprune map[module.Version]bool) (*ModuleGraph, error) {
	...
	if pruning == pruned {
		...
		readModGraphDebugOnce.Do(func() {
			for _, f := range strings.Split(os.Getenv("GODEBUG"), ",") {
				...
				case "lazymod=strict":
					debug.PrintStack()
					log.Fatalf("go: read full module graph (forbidden by GODEBUG=lazymod=strict).")
				}
			}
		})
	}
	...
	var (
		...
		hasError bool
		mg       = &ModuleGraph{
			g: mvs2.NewGraph(cmpVersion, graphRoots),
		}
	)
	...
	var (
		loadQueue = par2.NewQueue(runtime.GOMAXPROCS(0))
...
func (mg *ModuleGraph) findError() error {
	errStack := mg.g.FindPath(func(m module.Version) bool {
		...
		return err != nil && err != par2.ErrCacheEntryNotFound
	})
	if len(errStack) > 0 {
		_, err := mg.loadCache.Get(errStack[len(errStack)-1])
		var noUpgrade func(from, to module.Version) bool
		return mvs2.NewBuildListError(err, errStack, noUpgrade)
	}
...
func tidyPrunedRoots(ctx context.Context, mainModule module.Version, old *Requirements, pkgs []*loadPkg) (*Requirements, error) {
	...
	q := par2.NewQueue(runtime.GOMAXPROCS(0))
	for {
...
func spotCheckRoots(ctx context.Context, rs *Requirements, mods map[module.Version]bool) bool {
	...
	work := par2.NewQueue(runtime.GOMAXPROCS(0))
	for m := range mods {
		m := m
		work.Add(func() {
			if ctx.Err() != nil {
...
func tidyUnprunedRoots(ctx context.Context, mainModule module.Version, old *Requirements, pkgs []*loadPkg) (*Requirements, error) {
	...
	min, err := mvs2.Req(mainModule, rootPaths, reqs)
	if err != nil {
		return nil, err
	}
	buildList, err := mvs2.BuildList([]module.Version{mainModule}, reqs)
	if err != nil {
		return nil, err
	}
...
func updateUnprunedRoots(ctx context.Context, direct map[string]bool, rs *Requirements, add []module.Version) (*Requirements, error) {
	...
	for _, mainModule := range MainModules.Versions() {
		min, err := mvs2.Req(mainModule, rootPaths, &mvsReqs{roots: keep})
		if err != nil {
			return rs, err
		}
