...
import (
	...
	"github.com/inovacc/moonlight/pkg/module/internal/modindex"
...
func matchPackages(ctx context.Context, m *search.Match, tags map[string]bool, filter stdFilter, modules []module.Version) {
	...
	walkPkgs := func(root, importPathRoot string, prune pruning) {
		...
		root = util.WithFilePathSeparator(filepath.Clean(root))
		err := filepath.Walk(root, func(pkgDir string, fi fs.FileInfo, err error) error {
			if err != nil {
				m.AddError(err)
				return nil
			}
			...
			if !fi.IsDir() {
				if fi.Mode()&fs.ModeSymlink != 0 && want && strings.Contains(m.Pattern(), "...") {
					if target, err := os.Stat(pkgDir); err == nil && target.IsDir() {
