...
import (
	...
	modfetch2 "github.com/inovacc/moonlight/pkg/module/internal/modfetch"
	...
	"github.com/inovacc/moonlight/pkg/module/internal/search"
	"github.com/inovacc/moonlight/pkg/module/internal/util"
	"io/fs"
	"os"
	"path/filepath"
	"slices"
...
func Query(ctx context.Context, path, query, current string, allowed AllowedFunc) (*modfetch2.RevInfo, error) {
...
func queryReuse(ctx context.Context, path, query, current string, allowed AllowedFunc, reuse map[module.Version]*ModulePublic) (*modfetch2.RevInfo, error) {
	var info *modfetch2.RevInfo
	err := modfetch2.TryProxies(func(proxy string) (err error) {
...
func checkReuse(ctx context.Context, m module.Version, old *codehost.Origin) error {
	return modfetch2.TryProxies(func(proxy string) error {
...
func (queryDisabledError) Error() string {
	...
	//}
	return fmt.Sprintf("cannot query module due to -mod=")
}

func queryProxy(ctx context.Context, proxy, path, query, current string, allowed AllowedFunc, reuse map[module.Version]*ModulePublic) (*modfetch2.RevInfo, error) {
	...
	revWithOrigin := func(rev *modfetch2.RevInfo) *modfetch2.RevInfo {
		if rev == nil {
			if origin == nil {
				return nil
			}
			return &modfetch2.RevInfo{Origin: origin}
		}
	...
	lookup := func(v string) (*modfetch2.RevInfo, error) {
...
func QueryPattern(ctx context.Context, pattern, query string, current func(string) string, allowed AllowedFunc) (pkgMods []QueryResult, modOnly *QueryResult, err error) {
	...
	for _, mainModule := range MainModules.Versions() {
		...
		if len(m.Pkgs) > 0 {
			...
			return []QueryResult{{
				Mod:      mainModule,
				Rev:      &modfetch2.RevInfo{Version: mainModule.Version},
				Packages: m.Pkgs,
			}}, nil, nil
		}
	...
	err = modfetch2.TryProxies(func(proxy string) error {
...
func versionHasGoMod(_ context.Context, m module.Version) (bool, error) {
	...
	isFake := bytes.Equal(data, modfetch2.LegacyGoMod(m.Path))
	return !isFake, nil
}
...
type versionRepo interface {
	...
	Versions(ctx context.Context, prefix string) (*modfetch2.Versions, error)
	Stat(ctx context.Context, rev string) (*modfetch2.RevInfo, error)
	Latest(context.Context) (*modfetch2.RevInfo, error)
}

var _ versionRepo = modfetch2.Repo(nil)
...
func lookupRepo(ctx context.Context, proxy, path string) (repo versionRepo, err error) {
	...
	if err == nil {
		repo = modfetch2.Lookup(ctx, proxy, path)
	} else {
...
func (er emptyRepo) Versions(ctx context.Context, prefix string) (*modfetch2.Versions, error) {
	return &modfetch2.Versions{}, nil
}
func (er emptyRepo) Stat(ctx context.Context, rev string) (*modfetch2.RevInfo, error) {
	return nil, er.err
}
func (er emptyRepo) Latest(ctx context.Context) (*modfetch2.RevInfo, error) { return nil, er.err }
...
func (rr *replacementRepo) Versions(ctx context.Context, prefix string) (*modfetch2.Versions, error) {
	repoVersions, err := rr.repo.Versions(ctx, prefix)
	if err != nil {
		if !errors.Is(err, os.ErrNotExist) {
			return nil, err
		}
		repoVersions = new(modfetch2.Versions)
	}
	...
	util.Uniq(&versions)
	return &modfetch2.Versions{List: versions}, nil
}

func (rr *replacementRepo) Stat(ctx context.Context, rev string) (*modfetch2.RevInfo, error) {
...
func (rr *replacementRepo) Latest(ctx context.Context) (*modfetch2.RevInfo, error) {
...
func (rr *replacementRepo) replacementStat(v string) (*modfetch2.RevInfo, error) {
	rev := &modfetch2.RevInfo{Version: v}
