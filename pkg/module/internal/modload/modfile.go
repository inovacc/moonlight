...
import (
	...
	"github.com/inovacc/moonlight/pkg/module/internal/lockedfile"
	modfetch2 "github.com/inovacc/moonlight/pkg/module/internal/modfetch"
	"github.com/inovacc/moonlight/pkg/module/internal/par"
	"log"
	"os"
	"path/filepath"
	"strings"
...
func ReadModFile(gomod string, fix modfile.VersionFixer) (data []byte, f *modfile.File, err error) {
	gomod = base2.ShortPath(gomod) // use short path in any errors
	if gomodActual, ok := base2.OverlayPath(gomod); ok {
	...
	if f.Go != nil && gover2.Compare(f.Go.Version, gover2.Local()) > 0 {
		toolchain := ""
		if f.Toolchain != nil {
			toolchain = f.Toolchain.Name
		}
		return nil, nil, &gover2.TooNewError{What: gomod, GoVersion: f.Go.Version, Toolchain: toolchain}
	}
	if f.Module == nil {
...
func pruningForGoVersion(goVersion string) modPruning {
	if gover2.Compare(goVersion, gover2.ExplicitIndirectVersion) < 0 {
...
func CheckRetractions(ctx context.Context, m module.Version) (err error) {
	...
	for _, r := range summary.retract {
		if gover2.ModCompare(m.Path, r.Low, m.Version) <= 0 && gover2.ModCompare(m.Path, m.Version, r.High) <= 0 {
			isRetracted = true
			if r.Rationale != "" {
		...
		if index := MainModules.Index(v); index != nil {
			if from, r, ok := replacement(mod, index.replace); ok {
				...
				if foundModRoot != "" && foundFrom != from && found != r {
					log.Fatalf("conflicting replacements found for %v in workspace modules defined by %v and %v",
...
func rawGoModSummary(m module.Version) (*modFileSummary, error) {
	if gover2.IsToolchain(m.Path) {
		if m.Path == "go" && gover2.Compare(m.Version, gover2.GoStrictVersion) >= 0 {
	...
	return rawGoModSummaryCache.Do(m, func() (*modFileSummary, error) {
		...
		if err != nil {
			return nil, module.VersionError(m, fmt.Errorf("parsing %s: %v", base2.ShortPath(name), err))
		}
		if f.Module != nil {
		...
		if summary.goVersion != "" && gover2.Compare(summary.goVersion, gover2.GoStrictVersion) >= 0 {
			if gover2.Compare(summary.goVersion, gover2.Local()) > 0 {
				return nil, &gover2.TooNewError{What: "module " + m.String(), GoVersion: summary.goVersion}
			}
...
func rawGoModData(m module.Version) (name string, data []byte, err error) {
	if m.Version == "" {
		...
		if gomodActual, ok := base2.OverlayPath(name); ok {
			// Don't lock go.mod if it's part of the overlay.
			// On Plan 9, locking requires chmod, and we don't want to modify any file
			// in the overlay. See #44700.
			data, err = os.ReadFile(gomodActual)
		...
		if err != nil {
			return "", nil, module.VersionError(m, fmt.Errorf("reading %s: %v", base2.ShortPath(name), err))
		}
	} else {
		if !gover2.ModIsValid(m.Path, m.Version) {
			// Disallow the broader queries supported by fetch.Lookup.
			log.Fatalf("go: internal error: %s@%s: unexpected invalid semantic version", m.Path, m.Version)
		}
		name = "go.mod"
		data, err = modfetch2.GoMod(context.TODO(), m.Path, m.Version)
	}
