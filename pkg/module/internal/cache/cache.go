...
// DiskCache A Cache is a package cache, backed by a file system directory tree.
...
func GetMmap(c Cache, id ActionID) ([]byte, Entry, error) {
	...
	md, err := mmap.Open(c.OutputFile(entry.OutputID))
	if err != nil {
		return nil, Entry{}, err
	}
	if int64(md.Len()) != entry.Size {
		return nil, Entry{}, &entryNotFoundError{Err: errors.New("file incomplete")}
	}

	data := make([]byte, md.Len())
	if _, err = md.ReadAt(data, 0); err != nil {
		return nil, Entry{}, err
	}
	return data, entry, nil
}
