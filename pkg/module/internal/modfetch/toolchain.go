...
func (r *toolchainRepo) Versions(ctx context.Context, prefix string) (*Versions, error) {
	...
	if v := gover2.Local(); !have[v] {
		list = append(list, goPrefix+v)
	}

	if r.path == "go" {
		sort.Slice(list, func(i, j int) bool {
			return gover2.Compare(list[i], list[j]) < 0
		})
	} else {
		sort.Slice(list, func(i, j int) bool {
			return gover2.Compare(gover2.FromToolchain(list[i]), gover2.FromToolchain(list[j])) < 0
		})
	}
	versions.List = list
	return versions, nil
...
func (r *toolchainRepo) Stat(ctx context.Context, rev string) (*RevInfo, error) {
	...
	if r.path == "go" && gover2.Compare(v, gover2.Local()) <= 0 {
	...
	if r.path == "toolchain" && v == gover2.Local() {
		return &RevInfo{Version: prefix + v}, nil
	}
...
func (r *toolchainRepo) Latest(ctx context.Context) (*RevInfo, error) {
	...
	var max string
	for _, v := range versions.List {
		if max == "" || gover2.ModCompare(r.path, v, max) > 0 {
			max = v
		}
	}
