...
type vcsCmd struct {
	...
	tagRE         *vcs.Regexp                                                                         // regexp to extract tag names from output of tags cmd
	branches      func(remote string) []string                                                        // cmd to list local branches
	branchRE      *vcs.Regexp                                                                         // regexp to extract branch names from output of tags cmd
	badLocalRevRE *vcs.Regexp                                                                         // regexp of names that must not be served out of local cache without doing fetch first
...
var vcsCmds = map[string]*vcsCmd{
	"hg": {
		...
		readZip: func(rev, subdir, remote, target string) []string {
			...
			return util.StringList("hg", "archive", "-t", "zip", "--no-decode", "-r", rev, "--prefix=prefix/", pattern, "--", target)
		},
	},
	...
	"bzr": {
		...
		readZip: func(rev, subdir, remote, target string) []string {
			...
			return util.StringList("bzr", "export", "--format=zip", "-r", rev, "--root=prefix/", "--", target, extra)
		},
	},
	...
	"fossil": {
		...
		readZip: func(rev, subdir, remote, target string) []string {
			...
			return util.StringList("fossil", "zip", "-R", ".fossil", "--name", "prefix", extra, "--", rev, target)
		},
	},
}
