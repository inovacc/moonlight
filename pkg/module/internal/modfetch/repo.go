...
import (
	...
	"errors"
	"fmt"
	codehost2 "github.com/inovacc/moonlight/pkg/module/internal/modfetch/codehost"
	"github.com/inovacc/moonlight/pkg/module/internal/par"
	"github.com/inovacc/moonlight/pkg/module/internal/vcs"
...
type Repo interface {
	...
	CheckReuse(ctx context.Context, old *codehost2.Origin) error
...
type Versions struct {
	Origin *codehost2.Origin `json:",omitempty"` // origin information for reuse
...
type RevInfo struct {
	...
	Origin *codehost2.Origin `json:",omitempty"` // provenance for reuse
}
...
func (lookupDisabledError) Error() string {
	...
	return fmt.Sprintf("module lookup disabled by -mod=")
}
...
func lookupCodeRepo(ctx context.Context, rr *vcs.RepoRoot) (codehost2.Repo, error) {
	code, err := codehost2.NewRepo(ctx, rr.VCS.Cmd, rr.Repo)
	if err != nil {
		var VCSError *codehost2.VCSError
		if errors.As(err, &VCSError) {
			return nil, err
		}
...
func (l *loggingRepo) CheckReuse(ctx context.Context, old *codehost2.Origin) (err error) {
	defer func() {
...
func (r errRepo) CheckReuse(ctx context.Context, old *codehost2.Origin) error    { return r.err }
