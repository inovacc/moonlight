...
import (
	...
	cache2 "github.com/inovacc/moonlight/pkg/module/internal/cache"
...
func moduleHash(modroot string, ismodcache bool) (cache2.ActionID, error) {
	...
	if !ismodcache {
		...
		return cache2.ActionID{}, ErrNotIndexed
	}

	h := cache2.NewHash("moduleIndex")
...
func dirHash(modroot, pkgdir string) (cache2.ActionID, error) {
	h := cache2.NewHash("moduleIndex")
	fmt.Fprintf(h, "modroot %s\n", modroot)
	fmt.Fprintf(h, "package %s %s %v\n", runtime.Version(), indexVersion, pkgdir)
	entries, err := base.ReadDir(pkgdir)
	if err != nil {
	...
	for _, info := range entries {
		...
		// See hashOpen in github.com/dyammarcano/go-project/cmd/go/internal/test/test.go for the
		// corresponding code.
...
func openIndexModule(modroot string, ismodcache bool) (*Module, error) {
	return mcache.Do(modroot, func() (*Module, error) {
		...
		data, _, err := cache2.GetMmap(cache2.Default(), id)
		if err != nil {
			...
			if err = cache2.PutBytes(cache2.Default(), id, data); err != nil {
				return nil, err
			}
		}
...
func relPath(path, modroot string) string {
	return util.TrimFilePathPrefix(filepath.Clean(path), filepath.Clean(modroot))
}
...
func (rp *IndexPackage) Import(bctxt build.Context, mode build.ImportMode) (p *build.Package, err error) {
	...
	inTestdata := func(sub string) bool {
		return strings.Contains(sub, "/testdata/") || strings.HasSuffix(sub, "/testdata") || util.HasPathPrefix(sub, "testdata")
	}
	var pkga string
	if !inTestdata(rp.dir) {
		...
		if ctxt.GOROOT != "" /*&& str.HasFilePathPrefix(p.Dir, cfg.GOROOTsrc) && p.Dir != cfg.GOROOTsrc*/ {
			...
			if pkga != "" {
				...
				if !p.Goroot || (p.ImportPath != "unsafe" && p.ImportPath != "builtin") {
