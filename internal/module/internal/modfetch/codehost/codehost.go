...
type Repo interface {
	...
	// Tags List lists all tags with the given prefix.
...
func (e *UnknownRevisionError) Error() string {
	return fmt.Sprintf("unknown revision %s", e.Rev)
}
func (_ *UnknownRevisionError) Is(err error) bool {
...
func WorkDir(ctx context.Context, typ, name string) (dir, lockfile string, err error) {
	...
	lockfile = fmt.Sprintf("%s.lock", dir)
	//if buildX {
	...
	if err == nil && err2 == nil && info.IsDir() {
		...
		if have != key {
			return "", "", fmt.Errorf("%s exists with wrong content (have %q want %q)", infoFile, have, key)
		}
		//if buildX {
	...
	if err = os.WriteFile(infoFile, []byte(key), 0666); err != nil {
		os.RemoveAll(dir)
		return "", "", err
	}
	return dir, lockfile, nil
...
func (e *RunError) Error() string {
	text := fmt.Sprintf("%s: %s", e.Cmd, e.Err.Error())
	stderr := bytes.TrimRight(e.Stderr, "\n")
	if len(stderr) > 0 {
		text += fmt.Sprintf(":\n\t%s", strings.ReplaceAll(string(stderr), "\n", "\n\t"))
	}
	if len(e.HelpText) > 0 {
		text += fmt.Sprintf("\n%s", e.HelpText)
	}
...
func RunWithStdin(ctx context.Context, dir string, stdin io.Reader, cmdline ...any) ([]byte, error) {
	...
	c.Env = append(c.Environ(), fmt.Sprintf("GIT_DIR=%s", dir))
