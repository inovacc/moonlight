...
import (
	...
	"errors"
	"fmt"
	gover2 "github.com/inovacc/moonlight/internal/module/internal/gover"
...
func (r *codeRepo) Zip(ctx context.Context, dst io.Writer, version string) error {
	...
	if gomod, err := r.code.ReadFile(ctx, rev, filepath.Join(subdir, "go.mod"), codehost.MaxGoMod); err == nil {
		goVers := gover2.GoModLookup(gomod, "go")
		if gover2.Compare(goVers, gover2.Local()) > 0 {
			return &gover2.TooNewError{What: r.ModulePath() + "@" + version, GoVersion: goVers}
		}
