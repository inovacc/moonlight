...
import (
	...
	"errors"
	"fmt"
	"github.com/inovacc/moonlight/internal/component/search"
	"github.com/inovacc/moonlight/internal/module/internal/base"
	"github.com/inovacc/moonlight/internal/module/internal/cfg"
	"github.com/inovacc/moonlight/internal/module/internal/util"
...
var (
	...
	// github.com/dyammarcano/go-project/cmd/go/internal/vcweb.Server.
...
type vcsPath struct {
	...
	regexp         *Regexp                             // compiled pattern for import path
...
func repoRootFromVCSPaths(importPath string, security web.SecurityMode, vcsPaths []*vcsPath) (*RepoRoot, error) {
	if util.HasPathPrefix(importPath, "example.net") {
	...
	for _, srv := range vcsPaths {
		if !util.HasPathPrefix(importPath, srv.pathPrefix) {
			continue
		}
...
func interceptVCSTest(repo string, vcs *Cmd, security web.SecurityMode) (repoURL string, ok bool) {
	...
	if vcs == vcsMod {
		...
		// requests will be intercepted at a lower level (in github.com/dyammarcano/go-project/cmd/go/internal/web).
		return "", false
	}
	...
	for _, host := range VCSTestHosts {
		if !util.HasPathPrefix(repo, host) {
			continue
		}
...
func matchGoImport(imports []metaImport, importPath string) (metaImport, error) {
	...
	for i, im := range imports {
		if !util.HasPathPrefix(importPath, im.Prefix) {
...
var vcsPaths = []*vcsPath{
	// GitHub
	{
		...
		regexp:     New(`^(?P<root>github\.com/[\w.\-]+/[\w.\-]+)(/[\w.\-]+)*$`),
		vcs:        "git",
		repo:       "https://{root}",
		check:      noVCSSuffix,
	},
	...
	// Bitbucket
	{
		...
		regexp:     New(`^(?P<root>bitbucket\.org/(?P<bitname>[\w.\-]+/[\w.\-]+))(/[\w.\-]+)*$`),
		vcs:        "git",
		repo:       "https://{root}",
		check:      noVCSSuffix,
	},
	...
	// IBM DevOps Services (JazzHub)
	{
		...
		regexp:     New(`^(?P<root>hub\.jazz\.net/git/[a-z0-9]+/[\w.\-]+)(/[\w.\-]+)*$`),
		vcs:        "git",
		repo:       "https://{root}",
		check:      noVCSSuffix,
	},
	...
	// Git at Apache
	{
		...
		regexp:     New(`^(?P<root>git\.apache\.org/[a-z0-9_.\-]+\.git)(/[\w.\-]+)*$`),
		vcs:        "git",
		repo:       "https://{root}",
	},
	...
	// Git at OpenStack
	{
		...
		regexp:     New(`^(?P<root>git\.openstack\.org/[\w.\-]+/[\w.\-]+)(\.git)?(/[\w.\-]+)*$`),
		vcs:        "git",
		repo:       "https://{root}",
	},
	...
	// chiselapp.com for fossil
	{
		...
		regexp:     New(`^(?P<root>chiselapp\.com/user/[A-Za-z0-9]+/repository/[\w.\-]+)$`),
		vcs:        "fossil",
		repo:       "https://{root}",
	},
	...
	// Must be last.
	{
		regexp:         New(`(?P<root>(?P<repo>([a-z0-9.\-]+\.)+[a-z0-9.\-]+(:[0-9]+)?(/~?[\w.\-]+)+?)\.(?P<vcs>bzr|fossil|git|hg|svn))(/~?[\w.\-]+)*$`),
		schemelessRepo: true,
	},
}
...
var vcsPathsAfterDynamic = []*vcsPath{
	// Launchpad. See golang.org/issue/11436.
	{
		...
		regexp:     New(`^(?P<root>launchpad\.net/((?P<project>[\w.\-]+)(?P<series>/[\w.\-]+)?|~[\w.\-]+/(\+junk|[\w.\-]+)/[\w.\-]+))(/[\w.\-]+)*$`),
		vcs:        "bzr",
		repo:       "https://{root}",
		check:      launchpadVCS,
	},
...
// on github.com/dyammarcano/go-project/cmd/go/internal/load. It just needs to satisfy load.ImportPathError.
