...
func indexModule(modroot string) ([]byte, error) {
	...
	root := util.WithFilePathSeparator(modroot)
...
func parseErrorFromString(s string) error {
	...
	if err := json.Unmarshal([]byte(s), &p); err != nil {
		log.Fatalf(`go: invalid parse error value in index: %q. This indicates a corrupted index. Run "go clean -cache" to reset the module cache.`, s)
	}
	if p.ErrorList != nil {
		return *p.ErrorList
	}
...
func importRaw(modroot, reldir string) *rawPackage {
	...
	entries, err := base.ReadDir(absdir)
	if err != nil {
		p.error = err.Error()
		return p
	}
