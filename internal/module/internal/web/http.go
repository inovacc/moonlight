...
import (
	...
	"errors"
	"fmt"
	"github.com/inovacc/moonlight/internal/module/internal/auth"
...
func interceptURL(u *url.URL) (*Interceptor, bool) {
	if !enableTestHooks {
		return nil, false
	}
...
func get(security SecurityMode, u *url.URL) (*Response, error) {
	...
	if enableTestHooks {
		...
		case "proxy.golang.org":
			if os.Getenv("TESTGOPROXY404") == "1" {
				...
				//if cfg.BuildX {
				//	fmt.Fprintf(os.Stderr, "# get %s: %v (%.3fs)\n", u.Redacted(), res.Status, time.Since(start).Seconds())
				//}
				return res, nil
			}
	...
	fetch := func(u *url.URL) (*http.Response, error) {
		...
		if security == Insecure && u.Scheme == "https" { // fail earlier
	...
	if u.Scheme == "" || u.Scheme == "https" {
		...
		*secure = *u
		...
		if err == nil {
			...
			//}
			if security != Insecure || u.Scheme == "https" {
	...
	if res == nil {
		...
		case "http":
			if security == SecureOnly {
				...
				//}
				return nil, fmt.Errorf("insecure URL: %s", u.Redacted())
			}
		case "":
		...
		default:
			...
			//}
			return nil, fmt.Errorf("unsupported scheme: %s", u.Redacted())
		}

		insecure := new(url.URL)
		*insecure = *u
...
//func openBrowser(u string) bool { return browser.Open(u) }
